                ...
                <tr v-for="(field, index) in fields" :key="index" class="border-b last:border-b-0">
                  ...
                  <td class="w-2/3 px-4 py-3">
                    <slot :name="`field-${field.key}`" :field="field" :data="data">
                      <template v-if="field.formatter && getNestedValue(data, field.key) !== undefined">
                        {{ field.formatter(getNestedValue(data, field.key)) }}
                      </template>
                      ...
                      <template v-else-if="field.type === 'currency' && field.formatter">
                        {{ field.formatter(getNestedValue(data, field.key)) }}
                      </template>
